...
*Updated with single-PNG icon method (replacing 4-size SVG approach) and Tohu.AI rebrand — June 2026.*
...
*Note: this document outlines 6 build-stage phases (Assess/Architect/Specify/Build/Deploy/Iterate) as the MVP execution track. The fuller AI Product Engineering Methodology used for Fast & Flawless OS and referenced in "Proven On" runs 10 phases (Discovery, AI Boardroom, Problem Assessment, Product Architecture, Data Model, Commercial Review, Technical Architecture, Build Brief, Development, Deployment). The 6-phase track below maps roughly: Phase 1 ASSESS covers Discovery + AI Boardroom + Problem Assessment; Phase 2 ARCHITECT covers Product Architecture; Phase 3 SPECIFY covers Data Model + Build Brief; Phase 4 BUILD covers Development; Phase 5 DEPLOY covers Deployment; Phase 6 ITERATE is ongoing. A full reconciliation/rewrite to make this one consistent 10-phase document is still pending.*
...
*Updated June 2026 during the Tohu.AI rebrand — replaces the earlier 4-size SVG method below with a simpler single-PNG approach.*

Every deployed app should have an icon so it can be saved to the home screen like a native app. One PNG file does the whole job — no separate sizes, no SVG-to-PNG conversion step.

**Step 1 — Describe the icon to Claude**

Tell Claude:
- Background colour
- Text or symbol (e.g. "spell out TOHU")
- Accent colour / colour-coding logic (e.g. one colour per build phase)
- Style (e.g. tāniko pattern background, minimal, bold, Lego-brick)

**Step 2 — Claude outputs all files already named correctly**

Claude produces:
- `icon-512.png` — single PNG, 512x512, already named correctly
- `manifest.json` — PWA configuration, referencing the one PNG at both 512x512 and 192x192
- `head-tags.html` — snippet to paste into your `<head>`

**Critical rule:** Claude always outputs files with the exact final filename. The user never renames anything. Renaming on mobile causes GitHub upload errors every time.

**Step 3 — Upload to GitHub**

Upload two files to your repo root via **Upload Files** (never the mobile editor):
- `icon-512.png`
- `manifest.json`

No conversion. No second image file. Browsers downscale the single 512px PNG to render the 192px home-screen slot, so the manifest just lists the same filename twice with different `sizes` values.

**If GitHub creates a new branch instead of committing to main:**
This happens when uploading via mobile. Tap **Compare & pull request** → **Create pull request** → scroll down → **Merge pull request** → **Confirm merge** → **Delete branch**. Files are now in main and Vercel redeploys automatically.
...
**Step 6 — Replace the old home screen shortcut**

If this app already had an icon, the old PWA install on your phone is tied to the old manifest. Remove the old home screen icon first, then re-add from the live URL so Android picks up the new one.

**The test:** Does your new app icon appear when saved to the home screen? If yes, it's a PWA.

**Why the manifest lists one file twice:** some Android launchers and Chrome versions fall back to a low-res or generic icon on the home screen tile if no 192x192 entry exists in the manifest, even though the install itself succeeds. Listing the same 512px file at both sizes avoids this without needing a second image asset.
...
**Tohu.AI** (rebrand of Manifest Smarter) — Personal OS and AI Product Creation Method showcase
- Rebuilt: June 2026
- Result: New icon set and identity — Lego-brick icon with 10 colour-coded studs, one per phase of the AI Product Creation Method (Discovery, AI Boardroom, Problem Assessment, Product Architecture, Data Model, Commercial Review, Technical Architecture, Build Brief, Development, Deployment)
- URL: tohu-ai.vercel.app
- Repo: Hinepuororangi/manifest-smarter (rename pending)
- Key learning: single 512px PNG referenced twice in manifest.json (512x512 and 192x192) replaces the older 4-size SVG-to-PNG conversion workflow — fewer files, no external conversion site needed, container can render PNG directly via PIL when SVG rasterization tools aren't available
...
| 13 | Claude outputs a single 512px PNG + manifest.json + head-tags in one go | Saves steps, no SVG-to-PNG conversion site needed |
| 14 | Manifest lists one PNG file twice (512x512 and 192x192 entries) | Some Android launchers fall back to low-res icons without an explicit 192 entry; same file works for both sizes |
| 15 | Discovery before architecture | The real product is always different from the first idea — Phase 0 proves it |
| 16 | If rsvg-convert/cairosvg aren't available in the build container, render the icon directly as PNG via PIL/Pillow | Avoids dependency on external conversion sites entirely; works offline |
